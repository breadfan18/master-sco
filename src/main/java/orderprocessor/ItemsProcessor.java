...
public class ItemsProcessor {
    ...
    void wghtReqdProcessor(Item i, int key, int keyNum, Map<Integer, Item> itemMap, Map<Integer, Object[]> orderMap){
        int maxTries = 0;
        while (true){
            try {
                double itemWeight = bp.getDoubleFromScanner("Enter weight: ");
                double totalWeightedItemPrice = bp.roundUpToTwoDecimals(itemWeight*i.getUnitPrice());
                ...
                if (xcep.hasElecCpn(itemMap, key)){
                    ...
                    orderMap.put(keyNum, new Object[] {i.getItemName(), i.getUnitPrice(), itemWeight + "lb", totalWeightedItemPrice, eCpnNA, dollarWeightedItemPrice});
                    break;
                }
    ...
    void quantityReqdProcessor(Item i, int key, int keyNum, Map<Integer, Item> itemMap, Map<Integer, Object[]> orderMap){
        int maxTries = 0;
        while (true){
            try {
                int itemQuantity = bp.getIntFromScanner("Enter quantity: ");
                double totalQuantityItemPrice = bp.roundUpToTwoDecimals(itemQuantity*i.getUnitPrice());
                ...
                if (xcep.hasElecCpn(itemMap, key)){
                    ...
                    orderMap.put(keyNum, new Object[]{i.getItemName(), i.getUnitPrice(), itemQuantity, totalQuantityItemPrice, eCpnNA, dollarQuantityItemPrice});
                    break;
                }
            } catch (Exception e) {
    ...
    boolean ageRestrProcessor(Item i, int key, int keyNum, String itemUnit, Map<Integer, Item> itemMap, Map<Integer, Object[]> orderMap){
        boolean invalidDob = true;
        int maxTries = 0;
        while (invalidDob){
            try {
                String ageEntered = bp.getStringFromScanner("\tEnter DOB in 'mm-dd-yyyy' format: ");
                double ageRstrItemPrice = bp.roundUpToTwoDecimals(i.getUnitPrice());
    ...
    void normalItemProcessor(Item i, int key, int keyNum, String itemUnit, Map<Integer, Item> itemMap, Map<Integer, Object[]> orderMap){
        double normalItemPrice = bp.roundUpToTwoDecimals(i.getUnitPrice());
        ...
        if (xcep.hasElecCpn(itemMap, key)){
            ...
            String dollarCpnAdjustedPrice = bp.addDollarFormat(cpnAdjustedItemPrice);
            orderMap.put(keyNum, new Object[] {i.getItemName(), i.getUnitPrice(), itemUnit, cpnAdjustedItemPrice, "(" + eCpnAmt + ")", dollarCpnAdjustedPrice});
        }
        else {
            orderMap.put(keyNum, new Object[] {i.getItemName(), i.getUnitPrice(), itemUnit, normalItemPrice, eCpnNA, dollarNormalItemPrice});
        }
    }
